...
export default function cartReducer(state = {
    ...
    switch (action.type) {
        case ADD_TO_CART:
            ...
            if (state.cart.findIndex((product: any )=> product.id === productId) !== -1) {
        ...
        case DECREMENT_QTY:
            ...
            if (state.cart.findIndex((product: any )=> product.id === action.productId) !== -1) {
                const cart = state.cart.reduce((cartAcc: any, product: any) => {
        ...
        case REMOVE_FROM_CART:
            return {
                cart: state.cart.filter((item: any) => item.id !== action.product_id.id)
            }
